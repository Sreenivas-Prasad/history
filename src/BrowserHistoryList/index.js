...
class BrowserHistoryList extends React.Component {
  state = {searchInput: '', historyList: initialHistoryList}

  givingInputValue = event => {
    this.setState({searchInput: event.target.value})
  }

  getId = id => {
    const {historyList} = this.state
    const filteredList = historyList.filter(each1 => each1.id !== id)
        ...
        <div className="top">
          ...
          <div className="searching">
            ...
            <input
              ...
              onChange={this.givingInputValue}
            />
          </div>
        </div>
        <div className="middle">
          {searchResults.length !== 0 ? (
            ...
            <div className="display-m">
              <p className="para-no">There is no history to show</p>
            </div>
          )}
        </div>
      </div>
